...
export class AuthService {
  ...
  async login(dto:  LoginDto, meta?: { userAgent?: string; ip?: string }) {
    ...
    const session = await this.prisma.session.create({
      data: {
        ...
        refreshTokenExp: new Date(Date.now() + this.ms(process.env.JWT_REFRESH_TTL || '14d')),
  ...
  async refresh(dto: RefreshDto) {
    ...
    if (!session || session.revokedAt) throw new ForbiddenException('Invalid session');

    // Check expiry in DB as extra gate
    if (session.refreshTokenExp < new Date()) throw new ForbiddenException('Session expired');
